...
public class ChunkManager implements LoadingCallback {
	...
	public void ticketsLoaded(List<Ticket> tickets, World world) {
		...
		log("Starts to load chunks when world loading / %s.", world.provider.dimensionId);
		...
		log("Ends of loading chunks when world loading / %s.", world.provider.dimensionId);
	...
	public static String requestLabel(World world) {
		...
		log("Starts to release a new ticket of %s.", world.provider.dimensionId);
		...
		if(newTicket == null) {
			...
			log("Ends of releasing a new ticket of %s", world.provider.dimensionId);
			return null;
		}
	...
	public static boolean loadChunks(World world, int posX, int posY, int posZ, int range, String label) {
		...
		log("Starts to load chunks / %s.", world.provider.dimensionId);
		
		//	check argument and ticket.
		if(range < 0) {
			log("Invalid range.");
			log("Ends of loading chunks / %s.", world.provider.dimensionId);
			return false;
		}
	...
	public static void unloadChunks(World world, String label) {
		...
		log("Starts to unload chunks / %s.", world.provider.dimensionId);
		
		HashMap<String, Ticket> ticketsList = bundledTicketsLists.get(world);
		if(ticketsList == null) {
			log("No tickets list.");
			log("Ends of unloading chunks / %s.", world.provider.dimensionId);
			return;
		}
	...
	public static void releaseTicket(World world, String label) {
		...
		log("Starts to release a ticket / %s.", world.provider.dimensionId);
		
		HashMap<String, Ticket> ticketsList = bundledTicketsLists.get(world);
		if(ticketsList == null) {
			log("No tickets list.");
			log("Ends of releasing the ticket / %s.", world.provider.dimensionId);
			return;
		}
	...
	public static void checkIllegalChunks(World world) {
		...
		for(Map.Entry<String, Ticket> e: ticketsList.entrySet()) {
			...
			if(world.getBlock(posX, posY, posZ) != WakingChunksData.chunkAwaker
				&& WakingChunksData.ccLoaded
				&& world.getBlock(posX, posY, posZ) != WakingChunksData.cc_turtle
				&& world.getBlock(posX, posY, posZ) != WakingChunksData.cc_turtleExpanded
				&& world.getBlock(posX, posY, posZ) != WakingChunksData.cc_turtleAdvanced) {
